...
// ref https://github.com/jinzhu/copier
import (
	"database/sql"
	"database/sql/driver"
	"errors"
...
func copier(toValue interface{}, fromValue interface{}, opt Option) (err error) {
	...
	// from and to are Slice and array.
	if opt.CopySameItem && from.Kind() == reflect.Slice && to.Kind() == reflect.Slice {
		if fromType.ConvertibleTo(toType) {
			for i := 0; i < from.Len(); i++ {
				if to.IsNil() {
					to.Set(reflect.Append(to, from.Index(i)))
				}
				//  Find the key index for this array.
				resultIndexKey := findIndexByTag(from.Index(i), "key")
				fieldKeyName := ""
				resultIndexObjectID := 0
				var objectKey interface{}
				var objectFrom interface{}
				fieldObjectIDName := "ID"
				if resultIndexKey != -1 {
					fieldKeyName = fromType.Field(resultIndexKey).Name
					objectKey = reflect.Indirect(from.Index(i)).FieldByName(fieldKeyName).Interface()
					resultIndexObjectID = findIndexByTag(reflect.ValueOf(objectKey), "objectid")
					if resultIndexObjectID != -1 {
						fieldObjectIDName = reflect.ValueOf(objectKey).Type().Field(resultIndexObjectID).Name
					}
					objectFrom = reflect.Indirect(from.Index(i)).FieldByName(fieldKeyName).FieldByName(fieldObjectIDName).Interface()
				} else {
					// Key in not nested.
					fromType := reflect.Indirect(from.Index(i)).Type()
					resultIndexObjectID = findIndexByTag(from.Index(i), "objectid")
					// resultIndexObjectID = findIndexByTag(reflect.ValueOf(fromType), "objectid")
					if resultIndexObjectID != -1 {
						fieldObjectIDName = fromType.Field(resultIndexObjectID).Name
					}
					objectFrom = reflect.Indirect(from.Index(i)).FieldByName(fieldObjectIDName).Interface()
				}
				var result bool
				// Find slice to match data.
				for j := 0; j < to.Len(); j++ {
					var objectTo interface{}
					if fieldKeyName == "" {
						objectTo = reflect.Indirect(to.Index(j)).FieldByName(fieldObjectIDName).Interface()
					} else {
						objectTo = reflect.Indirect(to.Index(j)).FieldByName(fieldKeyName).FieldByName(fieldObjectIDName).Interface()
					}
					if reflect.DeepEqual(objectTo, objectFrom) {
						result = true
						isSet, err := set(to.Index(j), from.Index(i), opt.DeepCopy, converters)
						if err != nil {
							return err
						}
						if !isSet {
							// ignore error while copy slice element
							err = copier(to.Index(j).Addr().Interface(), from.Index(i).Interface(), opt)
							if err != nil {
								continue
							}
						}
				...
				//	When EmptySlice, append to slice.
				if !result {
					to.Set(reflect.Append(to, from.Index(i)))
				}
			}
	...
	// From or to are not struct.
	if fromType.Kind() != reflect.Struct || toType.Kind() != reflect.Struct {
		// skip not supported type
		return
	}

	// From or to are Slice and array
	...
	for i := 0; i < amount; i++ {
			...
			for _, field := range deepFields(toType) {
				...
				if fromMethod.IsValid() && fromMethod.Type().NumIn() == 0 && fromMethod.Type().NumOut() == 1 && !shouldIgnore(fromMethod, opt.IgnoreEmpty) {
					if toField := dest.FieldByName(destFieldName); toField.IsValid() && toField.CanSet() {
						...
						if len(values) >= 1 {
							if _, err := set(toField, values[0], opt.DeepCopy, converters); err != nil {
								return err
							}
						}
					}
		...
		if err = checkBitFlags(flgs.BitFlags); err != nil {
			return err
		}
	}
...
func isTag(tag string, tagFind string) bool {
	for _, t := range strings.Split(tag, ",") {
		if t == tagFind {
			return true
		}
...
func findIndexByTag(data reflect.Value, tagFind string) int {
	for i := 0; i < reflect.Indirect(data).Type().NumField(); i++ {
		field := reflect.Indirect(data).Type().Field(i)
		if isFind := isTag(field.Tag.Get("copier"), tagFind); isFind {
			return i
		}
	}
	return -1
